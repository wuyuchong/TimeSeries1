...
When $\theta=-1$, $Y_t$ depends negatively on $e_{t-1}$. The sample variance of the plot looks constant, which aligneds with theory since the true variance is $1*(1+(-1)^2)= 2$. Moreover, there is no trend in our data, but rather a choppy spread centered around 0. The reason for this is the nature of $e_t$ as an independent, identically distributed random variable. When turning to the correlograms, they look as expected. The sample ACF has a spike statistically different from 0 at k=1 (where the true 
...
When $\phi_1=-0.95$ we have a rather choppy time series plot. The variance varies greatly from time period to time period, and $Y_t$ looks centered around zero, wich would indicate nonstationarity. However $\phi_1$ is still smaller than 1, prooving the process to be covariance stationarity.
...
Wich makes it evident that $Y_t$ depends both on itself in the past period, as well as white noise in the current and past period.
...
In our third cell $\phi = 0.9$. Here we have an interesting change, as our plot has stopped being constant. Instead we have a very shoppy plot, which huges spikes and valleys. The wild variation of the plot does suggest that the variance might not be centered. It does look like the plot is centered around zero. The ACF is declining towards zero pretty fast, and does not oscillate. The PACF also goes to zero after two "fingers". The values of $\phi$ is very close to 1, which might make it very close to being nonstationary.
