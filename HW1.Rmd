...
This will be done in the following. We will discuss in total 6 different ARMA(1,1) processes; we have six diffrent cells for ARMA(1,1), which means six different plots to interpret. We use the variables $\theta$ and $\phi$. We expect both the ACF and PACF to be geometrically declining.


Table 11
In our first table, table 11, we have $\theta = -0.4$. In the first cell, $\phi=-0.9$. Our plot looks like it hits a point of permanent decline at around 450, but is constant before that point (note however the scale on the Y axis; there is probably variation before 450, just that it is not visible here). The line is not centered around zero,  due to the large decline. In other words, the mean seems to vary over time, wich violates the first requirement for stationarity. The sample ACF is oscillating but declining properly.  However the speed of the decline is not that fast. Note how the ACF ressembles the ACF for an AR(1) process. The sample PACF also declines, but faster. Here we see a tendency to decline towards zero, as after 3 "fingers" no values are significant.

In our second cell $\phi=-0.8$. The plot looks exactly the same as the previous cell. The sample ACF looks like it declines faster to zero compared to the previous cell. PACF also declines to zero, but seems to have three "fingers", although the third one is very close to not being significant.

In our third cell $\phi = 0.9$. Here we have an interesting change. Instead we have a very choppy plot, with spikes and valleys reaching only between -6 and 6. It does look like the plot is centered around zero, and the variance looks fairly constant. Hence, this process could very well be stationary. The ACF is declining towards zero pretty fast, and does not oscillate. The PACF also goes to zero after two "fingers".
...
Our second cell the plots looks fairly regular and smooth compared to the last cell of table 11. It looks fairly stationary, since the mean and variance seems constant. The ACF and PACF look similar to the ones in table 11, although the ACF seem to be declining more slowly. The PACF has three "fingers" before it declines to zero.
